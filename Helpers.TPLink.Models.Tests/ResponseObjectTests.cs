...
namespace Helpers.TPLink.Models.Tests
{
	public class ResponseObjectTests
	{
		...
		[InlineData(
			...
			"1.0.16 Build 210205 Rel.163735", "amp", "003192e1a474", 18, 244162, 0)]
		public void SysInfo(string json, string expectedSoftwareVersion, string expectedAlias, string expectedPhysicalAddress, int? expectedMilliamps, int? expectedMilliVolts, int? expectedMilliwatts)
		{
			var o = json.Deserialize<Models.Generated.ResponseObject>();
			Assert.NotNull(o);
			Assert.NotNull(o!.system);
			Assert.NotNull(o.system.get_sysinfo);
			Assert.Equal(expectedSoftwareVersion, o.system.get_sysinfo.sw_ver);
		...
		[Theory]
		[InlineData(@"{""system"":{""set_relay_state"":{""err_code"":0}}}", 0)]
		[InlineData(@"{""system"":{""set_relay_state"":{""err_code"":1}}}", 1)]
		public void RelayState(string json, int expected)
		{
			var o = json.Deserialize<Models.Generated.ResponseObject>();
			Assert.NotNull(o);
			Assert.NotNull(o.system);
			Assert.NotNull(o.system.set_relay_state);
			Assert.Equal(expected, o.system.set_relay_state.err_code);
		}
	}
}
